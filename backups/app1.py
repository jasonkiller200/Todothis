...
def batch_add_todo():
    ...
    try:
        # 將 due_date 字串轉換為 datetime 物件並設定為 UTC 時區感知型
        due_date = datetime.fromisoformat(data['due_date']).replace(tzinfo=utc)
    except ValueError:
        return jsonify({'error': 'Invalid due_date format. Use ISO format (YYYY-MM-DD).'}), 400
...
def assign_tracking_task_to_todo(meeting_task_id):
    ...
    try:
        ...
        if assigned_to_user and assigned_to_user.email:
            ...
            send_mail(subject, body, assigned_to_user.email)
...
# Current Time
2025/8/22 下午5:23:07 (Asia/Taipei, UTC+8:00)

# Context Window Usage
217,189 / 1,048.576K tokens used (21%)
