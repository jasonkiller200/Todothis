...
# from yahoo_service import send_yahoo_email # 導入 Yahoo 服務 (已停用)
...
def add_todo():
    ...
    #     yahoo_sender_email = os.getenv('YAHOO_SENDER_EMAIL')
    #     yahoo_sender_password = os.getenv('YAHOO_SENDER_PASSWORD') # This should be an app password

    #     if yahoo_sender_email and yahoo_sender_password:
    #         subject = f"新任務指派：{todo.title}"
    #         body = f"""
    #         <p>您有一個新任務已指派給您：</p>
    #         <p><strong>任務標題：</strong> {todo.title}</p>
    #         <p><strong>任務描述：</strong> {todo.description}</p>
    #         <p><strong>預計完成日期：</strong> {todo.due_date.astimezone(timezone('Asia/Taipei')).strftime('%Y-%m-%d')}</p>
    #         <p><strong>指派人：</strong> {current_user.name}</p>
    #         <p>請登入系統查看詳細資訊。</p>
    #         """
    #         send_yahoo_email(yahoo_sender_email, yahoo_sender_password, target_user.email, subject, body)
    #     else:
    #         logging.warning("Yahoo sender email or password not configured. Email notification skipped.")
...
def batch_add_todo():
    ...
    for user_key in user_keys:
        ...
        if target_user.email:
            ...
            #     send_yahoo_email(yahoo_sender_email, yahoo_sender_password, target_user.email, subject, body)
            # else:
            #     logging.warning(f"Yahoo sender email or password not configured for {target_user.email}. Email notification skipped.")
...
def update_todo_status(todo_id):
    ...
    # 發送郵件通知給任務擁有者
    owner_user = db.session.get(User, todo.user_id)
    if owner_user and owner_user.email:
        subject = f"任務狀態更新：{todo.title} - {STATUS_TRANSLATIONS.get(new_status, new_status)}"
        body = f"""
        <p>您的任務 <strong>{todo.title}</strong> 狀態已更新。</p>
        <p><strong>原狀態：</strong> {STATUS_TRANSLATIONS.get(old_status, old_status)}</p>
        <p><strong>新狀態：</strong> {STATUS_TRANSLATIONS.get(new_status, new_status)}</p>
        <p><strong>任務描述：</strong> {todo.description}</p>
        <p><strong>預計完成日期：</strong> {todo.due_date.astimezone(timezone('Asia/Taipei')).strftime('%Y-%m-%d')}</p>
        """
        if new_status == TodoStatus.UNCOMPLETED.value and uncompleted_reason:
            body += f"<p><strong>未完成原因：</strong> {uncompleted_reason}</p>"
        body += "<p>請登入系統查看詳細資訊。</p>"
        # yahoo_sender_email = os.getenv('YAHOO_SENDER_EMAIL')
        # yahoo_sender_password = os.getenv('YAHOO_SENDER_PASSWORD') # This should be an app password

        # if yahoo_sender_email and yahoo_sender_password:
        #     send_yahoo_email(yahoo_sender_email, yahoo_sender_password, owner_user.email, subject, body)
        # else:
        #     logging.warning(f"Yahoo sender email or password not configured for {owner_user.email}. Email notification skipped.")
...
def add_meeting_task():
    ...
    try:
        ...
        expected_completion_date = datetime.fromisoformat(expected_completion_date_str).replace(tzinfo=utc) if expected_completion_date_str else None
...
def create_new_meeting_discussion():
    ...
    try:
        ...
        db.session.rollback()
        logging.error(f"創建新會議和討論議題時發生錯誤: {e}")
...
def agree_meeting_task(meeting_task_id):
    ...
    try:
        ...
        if meeting_task.controller_user_id:
            ...
            if controller and controller.email:
                subject = f"決議已同意：{meeting_task.meeting.subject}"
                body = f"""
                <p>會議決議 <strong>{meeting_task.meeting.subject}</strong> 中的任務 <strong>{meeting_task.task_description}</strong> 已由 <strong>{current_user.name}</strong> 同意並最終確定。</p>
                <p>請登入系統查看詳細資訊。</p>
                """
                # yahoo_sender_email = os.getenv('YAHOO_SENDER_EMAIL')
                # yahoo_sender_password = os.getenv('YAHOO_SENDER_PASSWORD') # This should be an app password
                ...
                # if yahoo_sender_email and yahoo_sender_password:
                #     send_yahoo_email(yahoo_sender_email, yahoo_sender_password, controller.email, subject, body)
                # else:
                #     logging.warning(f"Yahoo sender email or password not configured for {controller.email}. Email notification skipped.")
...
def assign_tracking_task_to_todo(meeting_task_id):
    ...
    try:
        ...
        if assigned_to_user and assigned_to_user.email:
            subject = f"會議任務指派：{new_todo.title}"
            body = f"""
            <p>您有一個新的會議任務已指派給您：</p>
            <p><strong>任務標題：</strong> {new_todo.title}</p>
            <p><strong>任務描述：</strong> {new_todo.description}</p>
            <p><strong>預計完成日期：</strong> {new_todo.due_date.astimezone(timezone('Asia/Taipei')).strftime('%Y-%m-%d')}</p>
            <p><strong>指派人：</strong> {assigner_user.name}</p>
            <p>請登入系統查看詳細資訊。</p>
            """
            # yahoo_sender_email = os.getenv('YAHOO_SENDER_EMAIL')
            # yahoo_sender_password = os.getenv('YAHOO_SENDER_PASSWORD') # This should be an app password
            ...
            # if yahoo_sender_email and yahoo_sender_password:
            #     send_yahoo_email(yahoo_sender_email, yahoo_sender_password, assigned_to_user.email, subject, body)
            # else:
            #     logging.warning(f"Yahoo sender email or password not configured for {assigned_to_user.email}. Email notification skipped.")
...
def export_meeting_tasks_pdf():
    ...
    if tracking_items:
        ...
        data = [['任務事項', '主辦者', '管制者', '預計完成日期', '實際完成日期', '狀態']]
        for task in tracking_items:
            assigned_to_name = db.session.get(User, task.assigned_to_user_id).name if task.assigned_to_user_id else 'N/A'
    ...
    if resolution_items:
        ...
        data = [['任務事項', '主辦者', '管制者', '開始執行日期', '狀態']]
        for task in resolution_items:
            assigned_to_name = db.session.get(User, task.assigned_to_user_id).name if task.assigned_to_user_id else 'N/A'
            controller_name = db.session.get(User, task.controller_user_id).name if task.controller_user_id else 'N/A'
            expected_date = task.expected_completion_date.astimezone(timezone('Asia/Taipei')).strftime('%Y-%m-%d') if task.expected_completion_date else '未設定'
            translated_status = STATUS_TRANSLATIONS.get(task.status, task.status)
...
def edit_user(user_id):
    ...
    if request.method == 'POST':
        ...
        if User.query.filter(User.email == user.email, User.id != user.id).first():
            flash('電子郵件已存在！', 'error')
            return render_template('edit_user.html', user=user, UserLevel=UserLevel, DEPARTMENT_STRUCTURE=DEPARTMENT_STRUCTURE, UNIT_TO_MAIN_DEPT_MAP=UNIT_TO_MAIN_DEPT_MAP)
...
# 每週一 00:05 執行任務轉移和歸檔，並設定寬限時間
scheduler.add_job(transfer_and_archive_todos, 'cron', day_of_week='mon', hour=0, minute=1, timezone=taiwan_tz, misfire_grace_time=60)

# 啟動排程器，確保無論如何啟動應用都會執行
scheduler.start()
# 註冊應用退出時關閉排程器的函數
