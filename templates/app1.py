...
from config import LEVEL_ORDER, DEPARTMENT_STRUCTURE, UNIT_TO_MAIN_DEPT_MAP
...
class User(db.Model):
    ...
    def can_access_user_data(self, target_user_key):
        ...
        current_level = level_hierarchy.get(self.level, 0)
        ...
        if current_level >= 4:
            ...
            if target_user and (target_user.department == self.department or 
                              level_hierarchy.get(target_user.level, 0) < current_level):
                return True
        
        # 其他層級只能看自己部門的下級
        if current_level >= 2:
            target_user = User.query.filter_by(user_key=target_user_key).first()
            if target_user and target_user.department == self.department and \
               level_hierarchy.get(target_user.level, 0) < current_level:
                return True
    ...
    def can_assign_to(self, target_user):
        ...
        # 管理員可以指派給任何人 (除了其他管理員)
        if self.level == 'admin':
            return target_user.level != 'admin'

        # 製造中心-協理可以指派給任何人 (除了管理員)
        if self.level == 'executive-manager':
            return target_user.level != 'admin'

        # 廠長、經理、副理的指派邏輯
        if self.level in ['plant-manager', 'manager', 'assistant-manager']:
            # 必須是層級低於自己的
            if target_level_value >= current_level_value:
                return False
            # 不能指派給管理員或協理
            if target_user.level in ['admin', 'executive-manager']:
                return False
        ...
        if self.level == 'deputy-section-chief':
            return (self.department == target_user.department and self.unit == target_user.unit) and \
                   target_level_value < current_level_value
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        user = User.query.get(session['user_id'])
        if not user or user.level not in ['admin', 'director']:
    ...
    if request.method == 'POST':
        ...
        if user.check_password(password):
            ...
            if user.failed_login_attempts >= 3:
                user.lock_account(30)  # 鎖定30分鐘
                flash('登入失敗次數過多，帳戶已被鎖定30分鐘', 'error')
            else:
...
def _build_organization_structure(all_users, user_todos_map, director):
    # 定義組織階層的順序，用於排序
    level_order = {
        'executive-manager': 0, 'plant-manager': 1, 'manager': 2,
        'assistant-manager': 3, 'section-chief': 4, 'deputy-section-chief': 5,
        'team-leader': 6, 'staff': 7
    }
    ...
    for user in all_users:
        ...
        user.completed_tasks = sum(1 for todo in user_current_todos if todo.status == 'completed')
        ...
        # 1. 分配部門主管 (廠長/經理/副理)
        if user.level in ['plant-manager', 'manager', 'assistant-manager']:
            # 避免重複添加 director
        ...
        # 2. 分配單位(課)內成員
            ...
            if user.level in ['section-chief', 'deputy-section-chief']:
                unit_data['management_team'].append(user)
            elif user.level == 'team-leader':
        ...
        # 3. 分配部門內無明確單位的成員 (主要針對品保部/採購部)
            ...
            # 將非主管級別的用戶放入
            if user.level not in ['manager', 'assistant-manager']:
...
def index():
    ...
    all_current_todos = Todo.query.filter_by(todo_type='current').all()
    user_todos_map = {}
    ...
    # 最高層級主管 (製造中心-協理)
    director = User.query.filter_by(level='executive-manager').first()
    if director:
        director_current_todos = user_todos_map.get(director.id, [])
        director.total_tasks = len(director_current_todos)
        director.completed_tasks = sum(1 for todo in director_current_todos if todo.status == 'completed')
    
    all_users = User.query.filter(User.level != 'admin').all()
...
def get_user_detail(user_key):
    ...
    current_todos = Todo.query.filter_by(user_id=user.id, todo_type='current').all()
    next_todos = Todo.query.filter_by(user_id=user.id, todo_type='next').all()
    ...
    return jsonify({
        ...
        'permissions': {
            ...
            'can_assign': current_user.level in ['admin', 'executive-manager', 'plant-manager', 'director', 'manager', 'section-chief'],
...
def get_weekly_report():
    current_user = get_current_user()
    if not current_user or current_user.level not in ['admin', 'director', 'manager']:
    ...
    # 查詢本週歸檔的任務
    weekly_archived_todos = ArchivedTodo.query.filter(
        ArchivedTodo.archived_at >= start_of_week,
        ArchivedTodo.archived_at <= end_of_week
    ).all()

    report_data = {}
    for todo in weekly_archived_todos:
...
def get_monthly_report():
    ...
    # 查詢本月歸檔的任務
    monthly_archived_todos = ArchivedTodo.query.filter(
        ArchivedTodo.archived_at >= start_of_month,
        ArchivedTodo.archived_at <= end_of_month
    ).all()

    report_data = {}
    for todo in monthly_archived_todos:
        user = User.query.get(todo.user_id)
        if user:
            if user.department not in report_data:
                report_data[user.department] = {
...
def update_todo_status(todo_id):
    ...
    if not new_status or new_status not in ['pending', 'in-progress', 'completed', 'uncompleted']:
    ...
    if new_status == 'uncompleted':
        # 記錄未完成事件
        history_entry = {
            'event_type': 'status_changed',
            'timestamp': datetime.utcnow().isoformat(),
            'actor': {'id': current_user.id, 'name': current_user.name, 'user_key': current_user.user_key},
            'details': {'old_status': old_status, 'new_status': 'uncompleted', 'reason': uncompleted_reason}
        }
        history.append(history_entry)
        todo.history_log = json.dumps(history)
        todo.status = 'in-progress' # 自動切換為進行中
    else:
        # 記錄狀態變更事件
        history_entry = {
            'event_type': 'status_changed',
...
def admin_users():
    users = User.query.filter(User.level != 'admin').all()  # 不顯示其他管理員帳號
...
def delete_user(user_id):
    ...
    try:
        ...
        return jsonify({'message': f'使用者 {user.name} 已刪除'})
    except Exception as e:
        db.session.rollback()
...
def reset_user_password(user_id):
    ...
    return jsonify({
        'message': f'使用者 {user.name} 的密碼已重置',
...
def init_sample_data():
    ...
    if User.query.count() == 0:
        ...
        # 建立系統管理員帳號
        users_data = [
            {'user_key': 'admin', 'name': '系統管理員', 'role': '系統管理員', 'department': '資訊部', 'unit': None, 'level': 'admin', 'avatar': '👨‍💻', 'email': 'admin@admin.com'},
            
            # 製造中心-協理
            {'user_key': 'exec_manager', 'name': '鍾協理', 'role': '製造中心-協理', 'department': '製造中心', 'unit': None, 'level': 'executive-manager', 'avatar': '👨‍💼', 'email': 'exec.manager@hartford.com.tw'},

            # 廠長
            {'user_key': 'plant_manager1', 'name': '莊廠長', 'role': '廠長', 'department': '製造中心', 'unit': '第一廠', 'level': 'plant-manager', 'avatar': '👨‍🏭', 'email': 'plant.manager1@hartford.com.tw'},
            {'user_key': 'plant_manager3', 'name': '陳廠長', 'role': '廠長', 'department': '製造中心', 'unit': '第三廠', 'level': 'plant-manager', 'avatar': '👨‍🏭', 'email': 'plant.manager3@hartford.com.tw'},

            # 經理
            {'user_key': 'manager_purchase', 'name': '張經理', 'role': '經理', 'department': '製造中心', 'unit': '採購物流部', 'level': 'manager', 'avatar': '👨‍💻', 'email': 'manager.purchase@hartford.com.tw'},
            {'user_key': 'manager_qa', 'name': '郭經理', 'role': '副理', 'department': '製造中心', 'unit': '品保部', 'level': 'manager', 'avatar': '👩‍🔬', 'email': 'manager.qa@hartford.com.tw'},

            # 課長
            {'user_key': 'section_chief_z1', 'name': '吳課長', 'role': '課長', 'department': '第一廠', 'unit': '裝一課', 'level': 'section-chief', 'avatar': '👨‍🔧', 'email': 'chief.z1@hartford.com.tw'},
            {'user_key': 'section_chief_z2', 'name': '鄭課長', 'role': '課長', 'department': '第一廠', 'unit': '裝二課', 'level': 'section-chief', 'avatar': '👨‍🔧', 'email': 'chief.z2@hartford.com.tw'},
            {'user_key': 'section_chief_z3', 'name': '徐課長', 'role': '課長', 'department': '第三廠', 'unit': '裝三課', 'level': 'section-chief', 'avatar': '👨‍🔧', 'email': 'chief.z3@hartford.com.tw'},
            {'user_key': 'section_chief_pg', 'name': '高課長', 'role': '課長', 'department': '品保部', 'unit': '品管課', 'level': 'section-chief', 'avatar': '👩‍🏭', 'email': 'chief.pg@hartford.com.tw'},

            # 副課長 (範例)
            {'user_key': 'deputy_chief_z1', 'name': '趙副課長', 'role': '副課長', 'department': '第一廠', 'unit': '裝一課', 'level': 'deputy-section-chief', 'avatar': '👨‍🔧', 'email': 'deputy.chief.z1@hartford.com.tw'},

            # 組長
            {'user_key': 'team_leader_z1_1', 'name': '錢組長', 'role': '組長', 'department': '第一廠', 'unit': '組件課', 'level': 'team-leader', 'avatar': '👨‍🏭', 'email': 'leader.z1.1@hartford.com.tw'},
            {'user_key': 'team_leader_z2_1', 'name': '孫組長', 'role': '組長', 'department': '第一廠', 'unit': '裝二課', 'level': 'team-leader', 'avatar': '👨‍🏭', 'email': 'leader.z2.1@hartford.com.tw'},
            {'user_key': 'team_leader_z3_1', 'name': '李組長', 'role': '組長', 'department': '第三廠', 'unit': '裝三課', 'level': 'team-leader', 'avatar': '👨‍🏭', 'email': 'leader.z3.1@hartford.com.tw'},

            # 作業員 (範例)
            {'user_key': 'staff_z1_1', 'name': '李作業員', 'role': '作業員', 'department': '第一廠', 'unit': '裝一課', 'level': 'staff', 'avatar': '👷', 'email': 'staff.z1.1@hartford.com.tw'},
            {'user_key': 'staff_scm', 'name': '王作業員', 'role': '作業員', 'department': '採購物流部', 'unit': '資材成本課', 'level': 'staff', 'avatar': '👷', 'email': 'staff.scm@hartford.com.tw'},
            {'user_key': 'staff_qa', 'name': '林作業員', 'role': '作業員', 'department': '品保部', 'unit': '品管課', 'level': 'staff', 'avatar': '👷', 'email': 'staff.qa@hartford.com.tw'},
        ]
        ...
        if exec_manager:
            todos_data = [
                {'title': '製造中心月度營運會議', 'description': '召開製造中心各廠、部主管會議，檢討月度績效', 'status': 'in-progress', 'todo_type': 'current', 'user_id': exec_manager.id},
                {'title': '新產線導入評估', 'description': '評估新產品線導入可行性與效益分析', 'status': 'pending', 'todo_type': 'next', 'user_id': exec_manager.id},
            ]
        ...
        if plant_manager1:
            todos_data = [
                {'title': '第一廠週生產會議', 'description': '召開第一廠各課主管會議，檢討週生產進度', 'status': 'in-progress', 'todo_type': 'current', 'user_id': plant_manager1.id},
                {'title': '第一廠設備維護計畫', 'description': '制定第一廠年度設備預防性維護計畫', 'status': 'pending', 'todo_type': 'next', 'user_id': plant_manager1.id},
            ]
            for todo_data in todos_data:
                todo = Todo(**todo_data)
                history_entry = {
        ...
        if section_chief_z1:
            todos_data = [
                {'title': '裝一課生產排程', 'description': '安排本週裝一課生產排程與人力配置', 'status': 'completed', 'todo_type': 'current', 'user_id': section_chief_z1.id},
                {'title': '裝一課品質改善專案', 'description': '推動裝一課品質改善專案，降低不良率', 'status': 'in-progress', 'todo_type': 'current', 'user_id': section_chief_z1.id},
            ]
            for todo_data in todos_data:
                todo = Todo(**todo_data)
                history_entry = {
        ...
        if team_leader_z1_1:
            todos_data = [
                {'title': '裝一課組別日報', 'description': '填寫裝一課組別每日生產進度報告', 'status': 'in-progress', 'todo_type': 'current', 'user_id': team_leader_z1_1.id},
            ]
            for todo_data in todos_data:
                todo = Todo(**todo_data)
                history_entry = {
...
def transfer_and_archive_todos():
    ...
    with app.app_context():
        print(f"[{datetime.now()}] Running weekly todo transfer and archive job...")
        
        # 1. 轉移下週計畫到本週進度
        next_week_todos = Todo.query.filter_by(todo_type='next').all()
        for todo in next_week_todos:
            todo.todo_type = 'current'
            # 記錄自動轉移事件
            history = json.loads(todo.history_log or '[]')
            history.append({
            ...
            db.session.add(todo)
        db.session.commit()
        print(f"Transferred {len(next_week_todos)} next week todos to current.")

        # 2. 歸檔已完成的本週進度
        completed_current_todos = Todo.query.filter_by(todo_type='current', status='completed').all()
        ...
        db.session.commit()
        print(f"Archived {len(completed_current_todos)} completed current todos.")
        print(f"[{datetime.now()}] Weekly todo job finished.")
